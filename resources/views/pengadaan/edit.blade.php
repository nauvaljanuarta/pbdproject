...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-12">
            <h2 class="mb-4">Edit Pengadaan</h2>
        </div>
    </div>
                ...
                <form action="{{ route('pengadaan.update', $pengadaan->idpengadaan) }}" method="POST">
                    ...
                    <div class="row mb-3">
                            ...
                            <select name="id_vendor" id="vendor_idvendor" class="form-control" required>
                                ...
                                @foreach($vendors as $vendor)
                                    <option value="{{ $vendor->idvendor }}"
                                        {{ $vendor->idvendor == $pengadaan->vendor_idvendor ? 'selected' : '' }}>
                                        {{ $vendor->nama_vendor }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="number" name="subtotal" id="subtotal_nilai" class="form-control"
                                value="{{ $pengadaan->subtotal_nilai }}" readonly>
                        </div>
                    </div>
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="number" id="ppn" class="form-control"
                                value="{{ $pengadaan->ppn }}" readonly>
                        </div>

                        <!-- Total Nilai -->
                        <div class="col-md-6">
                            <label for="total_nilai" class="form-label">Total Nilai</label>
                            <input type="number" id="total_nilai" class="form-control"
                                value="{{ $pengadaan->total_nilai }}" readonly>
                        </div>
                    </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered" id="detail-table">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>
                                        <select name="details[{{ $index }}][idbarang]"
                                                class="form-control barang-select" required
                                                onchange="updateHargaSatuan({{ $index }})">
                                            <option value="">Pilih Barang</option>
                                            @foreach($barangs as $barang)
                                                <option value="{{ $barang->idbarang }}"
                                                    data-harga="{{ $barang->harga }}"
                                                    {{ $barang->idbarang == $detail->idbarang ? 'selected' : '' }}>
                                                    {{ $barang->nama }}
                                                </option>
                                            @endforeach
                                        </select>
                                    </td>
                                    <td><input type="number" name="details[{{ $index }}][harga_satuan]"
                                               class="form-control harga_satuan" value="{{ $detail->harga_satuan }}" required readonly></td>
                                    <td><input type="number" name="details[{{ $index }}][jumlah]"
                                               class="form-control jumlah" value="{{ $detail->jumlah }}" required
                                               oninput="updateSubTotal({{ $index }})"></td>
                                    <td><input type="number" name="details[{{ $index }}][sub_total]"
                                               class="form-control sub_total" value="{{ $detail->sub_total }}" required readonly></td>
